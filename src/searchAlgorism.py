...
def winChance(board, man):
    ...
    for row in range(board.height):
        for column in range(board.width):
            ...
            try:
                t_score = checkWin(board.board[row][column], board.board[row][column+1], board.board[row][column+2], board.board[row][column+3], man)
                if t_score >= WIN:
                    return WIN
                score += t_score
...
# MinMax
def alphabetaPruning(board, player, alpha, beta, depth):
